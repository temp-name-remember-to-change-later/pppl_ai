...
# Takes n, a number, as an input, then returns a hex code corresponding to a shade of gray, depending on how high the number is.
# If n < 0, it returns black, and if n > 1, it returns white.
# For numbers in between, it returns a shade of gray.
def makeGray(n):
    if n > 1:
        return "#FFFFFF"
    elif n < 0:
...
class App(Frame):
    ...
    def __init__(self, network, master=None):
